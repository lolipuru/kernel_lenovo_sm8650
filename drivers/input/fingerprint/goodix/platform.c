...
int gf_parse_dts(struct gf_dev *gf_dev)
{
	...
	gf_dev->vreg = regulator_get(dev, "fp_vdd_vreg");
	if(gf_dev->vreg == NULL) {
		pr_err("fp_vdd_vreg get filed!\n");
		return -ENODEV;
	}
...
int gf_power_on(struct gf_dev *gf_dev)
{
	...
	if (rc) {
		pr_err("fp_vdd_vreg set filed!\n");
		return rc ;
	}
	rc = regulator_get_voltage(gf_dev->vreg) ;
	pr_info("gf_power_on regulator_value %d!!!\n", rc) ;
	mdelay(15);
...
int gf_power_off(struct gf_dev *gf_dev)
{
	...
	if (rc) {
		pr_err("%s,regulator_disable set filed!\n",__func__);
		return rc ;
	}
	pr_info("%s,regulator_disable set success!\n",__func__) ;
	/* TODO: add your power control here */

	return rc;
}
EXPORT_SYMBOL(gf_power_on);
