...
#define WAVEFORM_OFFSET 3
#define HAPTIC_HV_DRIVER_VERSION	"v1.6.0.1"
static uint64_t IC_REG = 0;
...
char aw_rtp_name[][AW_RTP_NAME_MAX] = {
	...
	{"102_Haptic_Right_RTP.bin"},
	{"103_Haptic_Right_RTP.bin"},
	{"104_Haptic_Right_RTP.bin"},
	{"105_Haptic_Right_RTP.bin"},
	{"106_Haptic_Right_RTP.bin"},
	{"107_Haptic_Right_RTP.bin"},
	{"108_Haptic_Right_RTP.bin"},
	{"109_Haptic_Right_RTP.bin"},
	{"110_Haptic_Right_RTP.bin"},
	{"111_Haptic_Right_RTP.bin"},
	{"112_Haptic_Right_RTP.bin"},
	{"113_Haptic_Right_RTP.bin"},
	{"114_Haptic_Right_RTP.bin"},
	{"115_Haptic_Right_RTP.bin"},
	{"116_Haptic_Right_RTP.bin"},
	{"117_Haptic_Right_RTP.bin"},
	{"118_Haptic_Right_RTP.bin"},
	{"119_Haptic_Right_RTP.bin"},
	{"120_Haptic_Right_RTP.bin"},
	{"121_Haptic_Right_RTP.bin"},
	{"122_Haptic_Right_RTP.bin"},
	{"123_Haptic_Right_RTP.bin"},
	{"124_Haptic_Right_RTP.bin"},
	{"125_Haptic_Right_RTP.bin"},
	{"126_Haptic_Right_RTP.bin"},
	{"127_Haptic_Right_RTP.bin"},
	{"128_Haptic_Right_RTP.bin"},
	{"129_Haptic_Right_RTP.bin"},
	{"130_Haptic_Right_RTP.bin"},
	{"131_Haptic_Right_RTP.bin"},
	{"132_Haptic_Right_RTP.bin"},
	{"133_Haptic_Right_RTP.bin"},
	{"134_Haptic_Right_RTP.bin"},
	{"135_Haptic_Right_RTP.bin"},
	{"136_Haptic_Right_RTP.bin"},
	{"137_Haptic_Right_RTP.bin"},
	{"138_Haptic_Right_RTP.bin"},
	{"139_Haptic_Right_RTP.bin"},
	{"140_Haptic_Right_RTP.bin"},
	{"141_Haptic_Right_RTP.bin"},
	{"142_Haptic_Right_RTP.bin"},
	{"143_Haptic_Right_RTP.bin"},
	{"144_Haptic_Right_RTP.bin"},
	{"145_Haptic_Right_RTP.bin"},
	{"146_Haptic_Right_RTP.bin"},
	{"147_Haptic_Right_RTP.bin"},
	{"148_Haptic_Right_RTP.bin"},
	{"149_Haptic_Right_RTP.bin"},
	{"150_Haptic_Right_RTP.bin"},
	{"151_AGT_RTP.bin"},
			       ...
			       struct ff_effect *old)
{
	...
	case FF_CONSTANT:
		...
		aw_info("FF_CONSTANT waveform id = %d", aw_haptic->index);
		break;
	case FF_PERIODIC:
		...
		aw_info("FF_PERIODIC waveform id = %d", wav_id);
		...
		} else if (wav_id > EFFECT_WAVE_ID_MIN && wav_id < EFFECT_WAVE_ID_MAX) {
			switch (wav_id) {
				case EFFECT_DOUBLE_VIB:
					aw_haptic->activate_mode = AW_RAM_LOOP_MODE;
					aw_haptic->duration = EFFECT_DOUBLE_VIB_1;
					aw_haptic->index = aw_haptic->ram.ram_num;
					break;
				case EFFECT_GALLERY_SELECTEI:
					aw_haptic->activate_mode = AW_RAM_LOOP_MODE;
					aw_haptic->duration = EFFECT_SHORT_VIB_1;
					aw_haptic->index = aw_haptic->ram.ram_num;
					break;
				default:
					aw_err("Unsupported wav_id:%d", wav_id);
					break;
			}
		} else if (wav_id >= EFFECT_WAVE_ID_MAX && wav_id <= wav_id_max) {
...
//    aw_haptic->ram.base_addr = 0x800;   /* don't merge this line for oem */
	switch(cmd)
	{
...
static irqreturn_t irq_handle(int irq, void *data)
{
	...
	struct aw_haptic *aw_haptic = data;
        ...
        {
            if(aw_haptic->func->get_irq_state(aw_haptic) == AW_IRQ_ALMOST_EMPTY)
            {
                aw_info("%s: fifo almost empty \n", __func__);
...
static void dual_haptic_effect(int effect_mode)
{
	...
	switch (effect_mode)
	{
		...
		case SHORT_D_VIB_RAM_MODE:
			...
			schedule_work(&right->vibrator_work);
			break;
		case LONG_L_VIB_RAM_MODE:
			left->activate_mode = AW_RAM_LOOP_MODE;
			left->func->set_repeat_seq(left, left->index);
			schedule_work(&left->vibrator_work);
			break;
		case LONG_R_VIB_RAM_MODE:
			right->activate_mode = AW_RAM_LOOP_MODE;
			right->func->set_repeat_seq(right, right->index);
			schedule_work(&right->vibrator_work);
			break;
		case LONG_D_VIB_RAM_MODE:
			left->activate_mode = AW_RAM_LOOP_MODE;
			left->func->set_repeat_seq(left, left->index);
			right->activate_mode = AW_RAM_LOOP_MODE;
			right->func->set_repeat_seq(right, right->index);
		...
		case RTP_L_VIB_MODE:
			...
			if ((left->rtp_file_num > 0) && (left->rtp_file_num < rtp_num_max)) {
				pr_info("%s: aw_rtp_name[%d]: %s\n", __func__, 
					left->rtp_file_num,  aw_rtp_name[left->rtp_file_num]);
				left->state = 1;
				schedule_work(&left->rtp_work);
			} else {
				pr_info("%s: rtp_file_num number error:%d\n", 
					__func__, left->rtp_file_num);
			}
			break;
		case RTP_R_VIB_MODE:
			right->func->play_stop(right);
			...
			if ((right->rtp_file_num > 0) && (right->rtp_file_num < rtp_num_max)) {
				pr_info("%s: aw_rtp_name[%d]: %s\n", __func__, 
					right->rtp_file_num,  aw_rtp_name[right->rtp_file_num]);
				right->state = 1;
				schedule_work(&right->rtp_work);
			} else {
				pr_info("%s: rtp_file_num number error:%d\n", 
					__func__, right->rtp_file_num);
			}
			break;
		case RTP_D_VIB_MODE:
			left->func->play_stop(left);
		...
		default:
			pr_err("dual haptic error effect mode = %d",
							effect_mode);
			break;
	}
}
...
static ssize_t double_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	...
	if(!left || !right){
		pr_err("%s: dual haptic support err!\n", __func__);
		return -1;
	}
...
/*****************************************************
*@para effect_id(10 decimal)
*@ram_para offset is 0(L) or 21(R) (or) 41(D)
* ram short vibrator : wave indexe = effect_id - offset
*1 <= effect_id < 21 SHORT_L_VIB_RAM_MODE
*22 <= effect_id <42 SHORT_R_VIB_RAM_MODE
*42 <= effect_id <62 SHORT_D_VIB_RAM_MODE
*
*@ram_para offset is 61(L) or 71(R) or 81(D)
* ram long vibrator : duration = (effect_id - offset())*500ms
*62 <= effect_id <72 LONG_L_VIB_RAM_MODE
*72 <= effect_id <82 LONG_R_VIB_RAM_MODE
*82 <= effect_id <92 LONG_D_VIB_RAM_MODE
*
*@rtp_para offset is 91(L) or 191(R) 291(D)
*rtp mode : rtp_bin index = effect_id - offset
*92 <= effect_id <392
*392<= effect_id <692
*692 <= effect_id <992
******************************************************/
static ssize_t double_store(struct device *dev,
				   ...
				   const char *buf, size_t count)
{
	...
	if (likely(sscanf(buf, " %d", &effect_id) == 1)) {
		pr_info("dual haptic effect id = %d", effect_id);
		mutex_lock(&aw_haptic->lock);
		hrtimer_cancel(&left->timer);
		hrtimer_cancel(&right->timer);

		if((effect_id <= 0) &&(effect_id > EFFECT_ID_MAX)){
			left->state = 0;
			right->state = 0;
			return count;
		}else if((effect_id >= SHORT_L_VIB_BASE) && 
						(effect_id < SHORT_R_VIB_BSAE)){	//short left vib mode

			left->state = 1;
			left->index = (effect_id < AW_RAM_SEQ_LIMIT ? 
				(effect_id+1-SHORT_L_VIB_BASE) : AW_RAM_SEQ_LIMIT);
			
			right->state = 0;
			right->index = 0;

			effect_mode = SHORT_L_VIB_RAM_MODE;
		}else if((effect_id >= SHORT_R_VIB_BSAE) &&
						(effect_id < SHORT_D_VIB_BASE)){//short right vib mode

			left->state = 0;
			left->index = 0;
			
			right->state = 1;
			right->index = ((effect_id-SHORT_R_VIB_BSAE) < AW_RAM_SEQ_LIMIT ?
				effect_id+1-SHORT_R_VIB_BSAE : AW_RAM_SEQ_LIMIT);

			effect_mode = SHORT_R_VIB_RAM_MODE;
		}else if((effect_id >= SHORT_D_VIB_BASE) &&
				(effect_id < LONG_L_VIB_BASE)){//short double vib mode

			left->state = 1;
			left->index = ((effect_id-SHORT_D_VIB_BASE) < AW_RAM_SEQ_LIMIT ?
				(effect_id+1-SHORT_D_VIB_BASE) : AW_RAM_SEQ_LIMIT);

			right->state = 1;
			right->index = ((effect_id-SHORT_D_VIB_BASE) < AW_RAM_SEQ_LIMIT ?
				(effect_id+1-SHORT_D_VIB_BASE) : AW_RAM_SEQ_LIMIT);

			effect_mode = SHORT_D_VIB_RAM_MODE;
		}else if((effect_id >= LONG_L_VIB_BASE) &&
				(effect_id < LONG_R_VIB_BASE)){//long left vib mode

			left->state = 1;
			//[TASK][PIKE-1770][vibrator]Update the vibration waveform of factory test
//[TASK][PIKE-2976][vibrator] Add gain value configuration for King glory scene by luoming0812 at 20220309
			left->index = 10;			//default wave for long vib
			//left->gain = 128;
			left->duration = (effect_id+1 - LONG_L_VIB_BASE) * AW_LONG_VIB_SLICE;

			right->state = 0;
			right->index = 0;
			//right->gain = 128;

			effect_mode = LONG_L_VIB_RAM_MODE;
		}else if((effect_id >= LONG_R_VIB_BASE) &&
				(effect_id < LONG_D_VIB_BASE)){//long right vib mode
			...
			right->index = 10;
			//right->gain = 128;
			right->duration = (effect_id+1 - LONG_R_VIB_BASE) * AW_LONG_VIB_SLICE;

			effect_mode = LONG_R_VIB_RAM_MODE;
		}else if((effect_id >= LONG_D_VIB_BASE) &&
				(effect_id < RTP_L_VIB_BASE)){//long double vib mode

			left->state = 1;
			left->index = 10;
			//left->gain = 128;
			left->duration = (effect_id+1 - LONG_D_VIB_BASE) * AW_LONG_VIB_SLICE;

			right->state = 1;
			right->index = 10;
			//right->gain = 128;
			right->duration = (effect_id+1 - LONG_D_VIB_BASE) * AW_LONG_VIB_SLICE;
			effect_mode = LONG_D_VIB_RAM_MODE;
		}else if((effect_id >= RTP_L_VIB_BASE) &&
				(effect_id < RTP_R_VIB_BASE)){//rtp left vib mode

			effect_mode = RTP_L_VIB_MODE;
			left->rtp_file_num = effect_id + WAVEFORM_OFFSET - RTP_L_VIB_BASE;

		}else if((effect_id >= RTP_R_VIB_BASE) &&
				(effect_id < RTP_D_VIB_BASE)){//rtp right vib mode

			effect_mode = RTP_R_VIB_MODE;
			right->rtp_file_num = effect_id + WAVEFORM_OFFSET - RTP_R_VIB_BASE;

		}else if((effect_id >= RTP_D_VIB_BASE) &&
				(effect_id < RTP_VIB_OFFSET)){//rtp double vib mode

			effect_mode = RTP_D_VIB_MODE;
			left->rtp_file_num = effect_id + WAVEFORM_OFFSET - RTP_D_VIB_BASE;
			right->rtp_file_num =  effect_id + WAVEFORM_OFFSET - RTP_D_VIB_BASE;
		}
		mutex_unlock(&aw_haptic->lock);
	}
	dual_haptic_effect(effect_mode);
	return count;
}

static ssize_t awinic_ic_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	if (IC_REG == AW86937_CHIPID)
		len += snprintf(buf + len, PAGE_SIZE - len, "awinic_ic = %s\n", AW86937_CHIP_ID);
	else
		len += snprintf(buf + len, PAGE_SIZE - len, "can not find awinic_ic \n");
	return len;
}
...
static DEVICE_ATTR_RW(double);
static DEVICE_ATTR_RO(awinic_ic);
#endif
...
static void tiktap_enable_mix_play(struct aw_haptic *aw_haptic, bool en_flag)
{
	...
	if (en_flag) {
		left->i2c->addr = left->record_i2c_addr;
		haptic_hv_i2c_write_bits(left, AW8693X_REG_SYSCTRL3,
							AW8693X_BIT_SYSCTRL3_SLOT_CHSEL_MASK,
							AW8693X_BIT_SYSCTRL3_SLOT_CHSEL_LEFT);
		haptic_hv_i2c_write_bits(left, AW8693X_REG_SYSCTRL5,
							AW8693X_BIT_SYSCTRL5_EN_BRO_ADDR_MASK,
							AW8693X_BIT_SYSCTRL5_EN_BRO_ADDR_ON);
		haptic_hv_i2c_write_bits(left, AW8693X_REG_SYSCTRL5,
							AW8693X_BIT_SYSCTRL5_BROADCAST_ADDR_MASK,
							AW8693X_BROADCAST_ADDR);
		reg_val = 0xa8;
		haptic_hv_i2c_write_bits(left, AW8693X_REG_SYSCTRL2, (~(1 << 7)), (1 << 7));

		haptic_hv_i2c_write_bits(right, AW8693X_REG_SYSCTRL3,
							AW8693X_BIT_SYSCTRL3_SLOT_CHSEL_MASK,
							AW8693X_BIT_SYSCTRL3_SLOT_CHSEL_RIGHT);
		/* en PLL */
		haptic_hv_i2c_write_bits(right, AW8693X_REG_SYSCTRL4,
							AW8693X_BIT_SYSCTRL4_EN_DLL_MASK,
							AW8693X_BIT_SYSCTRL4_EN_DLL_ON);
		haptic_hv_i2c_write_bits(right, AW8693X_REG_SYSCTRL5,
							AW8693X_BIT_SYSCTRL5_EN_BRO_ADDR_MASK,
							AW8693X_BIT_SYSCTRL5_EN_BRO_ADDR_ON);
		haptic_hv_i2c_write_bits(right, AW8693X_REG_SYSCTRL5,
							AW8693X_BIT_SYSCTRL5_BROADCAST_ADDR_MASK,
							AW8693X_BROADCAST_ADDR);
		reg_val = 0x68;
		haptic_hv_i2c_writes(right, AW8693X_REG_SYSCTRL2, &reg_val, AW_I2C_BYTE_ONE);

		/* change broadcast addr */
		left->i2c->addr = AW8693X_BROADCAST_ADDR;
		haptic_hv_i2c_write_bits(left, AW8693X_REG_RTPCFG1,
							AW8693X_BIT_RTPCFG1_TWORTP_EN_MASK,
							AW8693X_BIT_RTPCFG1_TWORTP_ENABLE);
	} else {
		/* close dual rtp */
		left->i2c->addr = AW8693X_BROADCAST_ADDR;
		haptic_hv_i2c_write_bits(left, AW8693X_REG_RTPCFG1,
							AW8693X_BIT_RTPCFG1_TWORTP_EN_MASK,
							AW8693X_BIT_RTPCFG1_TWORTP_DISABLE);
		/* close left motor INTN clk out and close right motor TRIG2 clk in */
		reg_val = 0x28;
		haptic_hv_i2c_writes(left, AW8693X_REG_SYSCTRL2, &reg_val, AW_I2C_BYTE_ONE);
		/* close EN DLL */
		haptic_hv_i2c_write_bits(left, AW8693X_REG_SYSCTRL4,
							AW8693X_BIT_SYSCTRL4_EN_DLL_MASK,
							AW8693X_BIT_SYSCTRL4_EN_DLL_OFF);
		/* close broadcast mode*/
		haptic_hv_i2c_write_bits(left, AW8693X_REG_SYSCTRL5,
							AW8693X_BIT_SYSCTRL5_EN_BRO_ADDR_MASK,
							AW8693X_BIT_SYSCTRL5_EN_BRO_ADDR_OFF);
		/* restore i2c addr */
		left->i2c->addr = left->record_i2c_addr;
		aw_info("restore i2c addr 0x%0x", left->i2c->addr);
	}
}
...
static void tiktap_work_routine(struct work_struct *work)
{
	...
	while (true && count--) {
		if (tiktap_buf->status == MMAP_BUF_DATA_VALID) {
			...
			if (aw_haptic->tiktap_mix_flag) {
				...
				aw_haptic->tiktap_mix_flag = false;
			}
			mutex_unlock(&aw_haptic->lock);
			return;
		}
	...
	while (true) {
		...
		glb_state_val = aw_haptic->func->get_glb_state(aw_haptic);
		if ((glb_state_val & AW_GLBRD_STATE_MASK) == AW_STATE_STANDBY) {
			aw_err("last write has %zu", tiktap_get_sys_msecs() - write_start);
...
static long tiktap_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case TIKTAP_RTP_MIX_MODE:
		/* aw_info("cmd = TIKTAP_RTP_MIX_MODE!"); */
		tiktap_clean_buf(aw_haptic, MMAP_BUF_DATA_INVALID);
		aw_haptic->tiktap_stop_flag = true;
		mutex_lock(&aw_haptic->lock);
...
static int tiktap_init(struct aw_haptic *aw_haptic)
{
	...
	if (of_device_is_compatible(aw_haptic->i2c->dev.of_node, "awinic,haptic_hv_l"))
		tiktap_config_proc = proc_create(AW_TIKTAP_PROCNAME, 0666, NULL, &tiktap_proc_ops);

	if (of_device_is_compatible(aw_haptic->i2c->dev.of_node, "awinic,haptic_hv_r"))
		tiktap_config_proc = proc_create(AW_TIKTAP_PROCNAME_R, 0666, NULL, &tiktap_proc_ops_r);
#else
	tiktap_config_proc = proc_create(AW_TIKTAP_PROCNAME, 0666, NULL, &tiktap_proc_ops);
#endif
...
#endif
{
	...
	aw_info("enter");
	aw_haptic->i2c->addr = aw_haptic->record_i2c_addr;
#ifdef TIMED_OUTPUT
...
static int __init aw_i2c_init(void)
{
	...
	pr_info("<%s>%s: LAPIS aw_haptic driver version %s\n", AW_I2C_NAME, __func__,
