...
static int aw8671x_startup(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
...
static int aw8671x_set_dai_sysclk(struct snd_soc_dai *dai, int clk_id, uint32_t freq, int dir)
{
	aw_info("freq=%d", freq);
...
static int aw8671x_hw_params(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params,
			     struct snd_soc_dai *dai)
{
	...
	/* check i2s cfg */
	haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_I2SCFG3, &tmp_val, AW_I2C_BYTE_ONE);
...
static int aw8671x_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_SYSST, &reg_val, AW_I2C_BYTE_ONE);
		aw_info("sysst=0x%02X", reg_val);
		haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_SYSST2, &reg_val, AW_I2C_BYTE_ONE);
		aw_info("sysst2=0x%02X", reg_val);
		haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_SYSER, &reg_val, AW_I2C_BYTE_ONE);
...
static void aw8671x_shutdown(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	aw_snd_soc_codec_t *codec = aw_get_codec(dai);
	struct aw_haptic *aw_haptic = aw_componet_codec_ops.aw_snd_soc_codec_get_drvdata(codec);
...
static int aw8671x_codec_write(aw_snd_soc_codec_t *codec, uint32_t reg, uint32_t value)
{
...
#ifdef KERNEL_OVER_4_19
static const struct snd_soc_component_driver soc_codec_dev_aw8671x = {
...
static int aw8671x_snd_soc_init(struct device *dev)
{
	...
	ret = aw_componet_codec_ops.aw_snd_soc_register_codec(dev, &soc_codec_dev_aw8671x,
							      dai, ARRAY_SIZE(aw8671x_dai));
	if (ret < 0) {
...
static int aw8671x_check_qualify(struct aw_haptic *aw_haptic)
{
	...
	aw_info("enter");
	ret = haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_EFCFG5, &reg_val, AW_I2C_BYTE_ONE);
	if (ret < 0)
		return ret;
	if (!(reg_val & 0x80)) {
...
{
	haptic_hv_i2c_writes(aw_haptic, AW8671X_REG_PLAYCFG2, &gain, AW_I2C_BYTE_ONE);
}
...
static void aw8671x_set_wav_seq(struct aw_haptic *aw_haptic, uint8_t wav, uint8_t seq)
{
	haptic_hv_i2c_writes(aw_haptic, AW8671X_REG_WAVCFG1 + wav, &seq, AW_I2C_BYTE_ONE);
}

static void aw8671x_set_wav_loop(struct aw_haptic *aw_haptic, uint8_t wav, uint8_t loop)
{
	uint8_t tmp = 0;
...
static void aw8671x_set_rtp_data(struct aw_haptic *aw_haptic, uint8_t *data, uint32_t len)
{
...
static void aw8671x_set_ram_addr(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8671X_REG_RAMADDRH, AW8671X_BIT_RAMADDRH_MASK,
				 (uint8_t)(aw_haptic->ram.base_addr >> 8));
	haptic_hv_i2c_writes(aw_haptic, AW8671X_REG_RAMADDRL, &ram_addr_l, AW_I2C_BYTE_ONE);
}

static void aw8671x_auto_brake_mode(struct aw_haptic *aw_haptic, bool flag)
{
	if (flag) {
...
static uint8_t aw8671x_get_glb_state(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_GLBRD5, &state, AW_I2C_BYTE_ONE);
...
static void aw8671x_play_go(struct aw_haptic *aw_haptic, bool flag)
{
	...
	if (flag) {
		...
		haptic_hv_i2c_writes(aw_haptic, AW8671X_REG_PLAYCFG4, &stop_on, AW_I2C_BYTE_ONE);
	}
}
...
static void aw8671x_play_mode(struct aw_haptic *aw_haptic, uint8_t play_mode)
{
	...
	case AW_RAM_MODE:
		...
		aw8671x_auto_brake_mode(aw_haptic, false);
	...
	case AW_RAM_LOOP_MODE:
		...
		haptic_hv_i2c_write_bits(aw_haptic, AW8671X_REG_PLAYCFG3,
					 AW8671X_BIT_PLAYCFG3_PLAY_MODE_MASK,
					 AW8671X_BIT_PLAYCFG3_PLAY_MODE_RAM);
		aw8671x_auto_brake_mode(aw_haptic, true);
...
static void aw8671x_stop(struct aw_haptic *aw_haptic)
{
	...
	aw_info("enter");
	aw_haptic->play_mode = AW_STANDBY_MODE;
	reg_val = AW8671X_BIT_PLAYCFG4_GO_ON;
	haptic_hv_i2c_write_bits(aw_haptic, AW8671X_REG_PLAYCFG3,
					AW8671X_BIT_PLAYCFG3_PLAY_MODE_MASK,
					AW8671X_BIT_PLAYCFG3_PLAY_MODE_STOP);
	haptic_hv_i2c_writes(aw_haptic, AW8671X_REG_PLAYCFG4, &reg_val, AW_I2C_BYTE_ONE);
	ret = aw8671x_wait_enter_standby(aw_haptic);
	if (ret < 0) {
...
static void aw8671x_upload_lra(struct aw_haptic *aw_haptic, uint32_t flag)
{
	...
	case AW_WRITE_ZERO:
		...
		haptic_hv_i2c_write_bits(aw_haptic, AW8671X_REG_TRIMCFG2,
					 AW8671X_BIT_TRIMCFG2_TRIM_LRA_MASK, 0x00);
		break;
	case AW_F0_CALI_LRA:
...
static uint8_t aw8671x_get_trim_lra(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_TRIMCFG2, &reg_val, AW_I2C_BYTE_ONE);
	reg_val &= 0x3F;
...
static void aw8671x_protect_config(struct aw_haptic *aw_haptic, uint8_t prtime, uint8_t prlvl)
{
	uint8_t reg_val = 0;

	haptic_hv_i2c_write_bits(aw_haptic, AW8671X_REG_PWMCFG1, AW8671X_BIT_PWMCFG1_PRC_EN_MASK,
	...
	if (prlvl != 0) {
		...
		haptic_hv_i2c_writes(aw_haptic, AW8671X_REG_PWMCFG3, &reg_val, AW_I2C_BYTE_ONE);
		haptic_hv_i2c_writes(aw_haptic, AW8671X_REG_PWMCFG4, &prtime, AW_I2C_BYTE_ONE);
	} else {
		/* Disable */
...
static void aw8671x_cont_config(struct aw_haptic *aw_haptic)
{
	...
	/* DRV2_TIME */
	haptic_hv_i2c_writes(aw_haptic, AW8671X_REG_CONTCFG9, &drv2_time, AW_I2C_BYTE_ONE);
	/* cont play go */
...
static void aw8671x_one_wire_init(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW8671X_REG_GLBCFG4, &trig_prio, AW_I2C_BYTE_ONE);
	haptic_hv_i2c_writes(aw_haptic, AW8671X_REG_GLBCFG2, &delay_2p5ms, AW_I2C_BYTE_ONE);
...
static void aw8671x_i2s_init(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8671X_REG_SYSCTRL2, AW8671X_BIT_SYSCTRL2_I2S_PIN_MASK,
...
static void aw8671x_trig1_param_config(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8671X_REG_TRGCFG1, (AW8671X_BIT_TRG_ENABLE_MASK &
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8671X_REG_TRGCFG4, (AW8671X_BIT_TRG_ENABLE_MASK &
				 AW8671X_BIT_TRG_SEQ_MASK), trig_config);
}
...
static void aw8671x_trig2_param_config(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8671X_REG_TRGCFG2, (AW8671X_BIT_TRG_ENABLE_MASK &
				 AW8671X_BIT_TRG_SEQ_MASK), trig_config);
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8671X_REG_TRGCFG5, (AW8671X_BIT_TRG_ENABLE_MASK &
				 AW8671X_BIT_TRG_SEQ_MASK), trig_config);
}
...
static void aw8671x_trig3_param_config(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8671X_REG_TRGCFG3, (AW8671X_BIT_TRG_ENABLE_MASK &
				 AW8671X_BIT_TRG_SEQ_MASK), trig_config);
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8671X_REG_TRGCFG6, (AW8671X_BIT_TRG_ENABLE_MASK &
				 AW8671X_BIT_TRG_SEQ_MASK), trig_config);
}
...
static void aw8671x_interrupt_setup(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_SYSINT, &reg_val, AW_I2C_BYTE_ONE);
...
static void aw8671x_get_ram_data(struct aw_haptic *aw_haptic, char *buf)
{
	...
	while (i < aw_haptic->ram.len) {
		...
		haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_RAMDATA, buf + i, size);
		i += size;
	}
}

static void aw8671x_get_first_wave_addr(struct aw_haptic *aw_haptic, uint8_t *wave_addr)
{
	uint8_t reg_array[3] = {0};

	haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_RAMDATA, reg_array, AW_I2C_BYTE_THREE);
...
static size_t aw8671x_get_wav_loop(struct aw_haptic *aw_haptic, char *buf)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_WAVCFG9, reg_val, AW_SEQUENCER_LOOP_SIZE);
...
static uint8_t aw8671x_get_prctmode(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_PWMCFG3, &reg_val, AW_I2C_BYTE_ONE);
	reg_val >>= 7;
...
static int aw8671x_read_f0(struct aw_haptic *aw_haptic)
{
	...
	/* lra_f0 */
	haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_CONTCFG14, reg_val, AW_I2C_BYTE_TWO);
	...
	f0_tmp = AW8671X_F0_FORMULA(f0_reg);
	aw_haptic->f0 = (uint32_t)f0_tmp;
	aw_info("lra_f0=%d", aw_haptic->f0);
#else
	/* cont_f0 */
	haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_CONTCFG16, reg_val, AW_I2C_BYTE_TWO);
	f0_reg = (reg_val[0] << 8) | reg_val[1];
	if (!f0_reg) {
		aw_haptic->f0 = 0;
...
static int aw8671x_get_f0(struct aw_haptic *aw_haptic)
{
	...
	/* config dts d2s_gain */
	haptic_hv_i2c_write_bits(aw_haptic, AW8671X_REG_DETCFG3, AW8671X_BIT_DETCFG3_D2S_GAIN_MASK,
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8671X_REG_CONTCFG6,
				 ...
				 (aw_haptic->info.is_enabled_track_en << 7));
	/* enable auto break */
	haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_PLAYCFG3, &reg_val, AW_I2C_BYTE_ONE);
	brk_en_default = reg_val & AW8671X_BIT_PLAYCFG3_BRK_ENABLE;
	aw8671x_auto_brake_mode(aw_haptic, true);
	/* f0 driver level & time */
	haptic_hv_i2c_write_bits(aw_haptic, AW8671X_REG_CONTCFG6,
				 AW8671X_BIT_CONTCFG6_DRV1_LVL_MASK, aw_haptic->info.cont_drv1_lvl);
	...
	haptic_hv_i2c_writes(aw_haptic, AW8671X_REG_CONTCFG7, cont_config, AW_I2C_BYTE_THREE);
	/* TRACK_MARGIN */
	...
	haptic_hv_i2c_writes(aw_haptic, AW8671X_REG_CONTCFG3, &cont_config[0], AW_I2C_BYTE_ONE);
	/* play go */
	aw8671x_play_go(aw_haptic, true);
...
static uint8_t aw8671x_get_osc_status(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_SYSST2, &state, AW_I2C_BYTE_ONE);
...
static void aw8671x_get_lra_resistance(struct aw_haptic *aw_haptic)
{
	...
	aw8671x_stop(aw_haptic);
	haptic_hv_i2c_write_bits(aw_haptic, AW8671X_REG_DETCFG3, AW8671X_BIT_DETCFG3_DET_SEQ0_MASK,
				 AW8671X_BIT_DETCFG3_DET_SEQ0_RL);
	/* ADC_FS */
	haptic_hv_i2c_reads(aw_haptic,  AW8671X_REG_DETCFG1, &reg_val[0], AW_I2C_BYTE_ONE);
	adc_fs_default = reg_val[0] & AW8671X_BIT_DETCFG1_ADC_FS;
	haptic_hv_i2c_write_bits(aw_haptic, AW8671X_REG_DETCFG1, AW8671X_BIT_DETCFG1_ADC_FS_MASK,
				 AW8671X_BIT_DETCFG1_ADC_FS_12KHZ);
	/* D2S_GAIN*/
	haptic_hv_i2c_reads(aw_haptic,  AW8671X_REG_DETCFG3, &reg_val[0], AW_I2C_BYTE_ONE);
	d2s_gain_default = reg_val[0] & AW8671X_BIT_DETCFG3_D2S_GAIN;
	haptic_hv_i2c_write_bits(aw_haptic, AW8671X_REG_DETCFG3, AW8671X_BIT_DETCFG3_D2S_GAIN_MASK,
				 AW8671X_BIT_DETCFG3_D2S_GAIN_10);
	d2s_gain = aw8671x_select_d2s_gain(AW8671X_BIT_DETCFG3_D2S_GAIN_10);
	if (d2s_gain <= 0) {
		aw_err("d2s_gain is error");
		return;
	}
	haptic_hv_i2c_write_bits(aw_haptic, AW8671X_REG_DETCFG1, AW8671X_BIT_DETCFG1_DET_GO_MASK,
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8671X_REG_DETCFG3, AW8671X_BIT_DETCFG3_D2S_GAIN_MASK,
				 d2s_gain_default);
	haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_DETRD1, reg_val, AW_I2C_BYTE_TWO);
	lra_code = ((reg_val[0] & AW8671X_BIT_DETRD1_AVG_DATA) << 8) + reg_val[1];
...
static void aw8671x_get_vbat(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8671X_REG_DETCFG1, AW8671X_BIT_DETCFG1_DET_GO_MASK,
				 AW8671X_BIT_DETCFG1_DET_GO_NA);
	haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_DETRD1, reg_val, AW_I2C_BYTE_TWO);
	vbat_code = ((reg_val[0] & AW8671X_BIT_DETRD1_AVG_DATA) << 8) + reg_val[1];
...
static ssize_t aw8671x_get_reg(struct aw_haptic *aw_haptic, ssize_t len, char *buf)
{
	uint8_t i = 0;
	uint8_t reg_array[AW8671X_REG_ANACFG14 + 1] = {0};

	haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_RSTCFG, reg_array, AW8671X_REG_RTPDATA);
	...
	haptic_hv_i2c_reads(aw_haptic, (AW8671X_REG_RAMDATA + 1),
			    ...
			    (AW8671X_REG_ANACFG14 - AW8671X_REG_RAMDATA));
...
static int aw8671x_offset_cali(struct aw_haptic *aw_haptic)
{
	return 0;
}
...
static int aw8671x_check_ram_data(struct aw_haptic *aw_haptic, uint8_t *cont_data,
				    ...
				    struct aw_haptic_container *awinic_cont)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW8671X_REG_RTPCFG1, rtp_addr, AW_I2C_BYTE_FIVE);
	haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_RTPCFG3, reg_val, AW_I2C_BYTE_THREE);
	...
	while (i < awinic_cont->len) {
		...
		haptic_hv_i2c_writes(aw_haptic, AW8671X_REG_RAMDATA, &awinic_cont->data[i], len);
		i += len;
	}
#ifdef AW_CHECK_RAM_DATA
	aw8671x_set_ram_addr(aw_haptic);
	...
	while (i < awinic_cont->len) {
		...
		haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_RAMDATA, ram_data, len);
		ret = aw8671x_check_ram_data(aw_haptic, &awinic_cont->data[i], ram_data, len);
		if (ret < 0)
			break;
		i += len;
	}
...
static uint64_t aw8671x_get_theory_time(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_SYSCTRL2, &reg_val, AW_I2C_BYTE_ONE);
			    ...
			    struct device_node *np)
{
	...
	val = of_property_read_u8(np, "aw8671x_gain_bypass", &aw_haptic->info.gain_bypass);
	if (val != 0)
		aw_info("aw8671x_gain_bypass not found");
	val = of_property_read_u32(np, "f0_pre", &aw_haptic->info.f0_pre);
	if (val != 0)
		aw_info("f0_pre not found");
	val = of_property_read_u8(np, "aw8671x_f0_cali_percent", &aw_haptic->info.f0_cali_percent);
	if (val != 0)
		aw_info("aw8671x_f0_cali_percent not found");
	val = of_property_read_u8(np, "aw8671x_cont_drv1_lvl", &aw_haptic->info.cont_drv1_lvl);
	if (val != 0)
		aw_info("aw8671x_cont_drv1_lvl not found");
	val = of_property_read_u32(np, "aw8671x_cont_lra_vrms", &aw_haptic->info.cont_lra_vrms);
	if (val != 0)
		aw_info("aw8671x_cont_lra_vrms not found");
	val = of_property_read_u8(np, "aw8671x_cont_drv1_time", &aw_haptic->info.cont_drv1_time);
	if (val != 0)
		aw_info("aw8671x_cont_drv1_time not found");
	val = of_property_read_u8(np, "aw8671x_cont_drv2_time", &aw_haptic->info.cont_drv2_time);
	if (val != 0)
		aw_info("aw8671x_cont_drv2_time not found");
	val = of_property_read_u8(np, "aw8671x_cont_brk_gain", &aw_haptic->info.cont_brk_gain);
	if (val != 0)
		aw_info("aw8671x_cont_brk_gain not found");
	val = of_property_read_u8(np, "aw8671x_d2s_gain", &aw_haptic->info.d2s_gain);
	if (val != 0)
		aw_info("aw8671x_d2s_gain not found");
	val = of_property_read_u8(np, "aw8671x_cont_brk_time", &aw_haptic->info.cont_brk_time);
	if (val != 0)
	...
	aw_haptic->info.is_enabled_i2s = of_property_read_bool(np, "aw8671x_is_enabled_i2s");
	...
	val = of_property_read_u32(np, "aw8671x_bst_vol_default", &aw_haptic->info.bst_vol_default);
	if (val != 0)
...
static void aw8671x_misc_para_init(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW8671X_REG_SYSCTRL4, reg_val, AW_I2C_BYTE_ONE);
	/* Cont drv2 lvl */
	drv2_lvl = AW8671X_DRV2_LVL_FORMULA(aw_haptic->info.f0_pre, aw_haptic->info.cont_lra_vrms);
	...
	/* Get gain */
	haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_PLAYCFG2, reg_val, AW_I2C_BYTE_ONE);
	aw_haptic->gain = reg_val[0];
	/* Get wave_seq */
	haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_WAVCFG1, reg_val, AW_I2C_BYTE_EIGHT);
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8671X_REG_CONTCFG5,
				 AW8671X_BIT_CONTCFG5_BRK_GAIN_MASK, aw_haptic->info.cont_brk_gain);
				  ...
				  const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		haptic_hv_i2c_reads(aw_haptic, AW8671X_REG_CONTCFG6, reg_array, AW_I2C_BYTE_ONE);
		...
		haptic_hv_i2c_writes(aw_haptic, AW8671X_REG_CONTCFG6, reg_array, AW_I2C_BYTE_TWO);
	}
				   ...
				   const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		haptic_hv_i2c_writes(aw_haptic, AW8671X_REG_CONTCFG8, reg_array, AW_I2C_BYTE_TWO);
	}
...
static int aw8671x_creat_node(struct aw_haptic *aw_haptic)
{
	...
	ret = sysfs_create_group(&aw_haptic->vib_dev.dev->kobj, &aw8671x_vibrator_attribute_group);
	if (ret < 0) {
