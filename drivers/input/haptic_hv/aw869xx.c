...
static int aw869xx_startup(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
...
static int aw869xx_set_fmt(struct snd_soc_dai *dai, uint32_t fmt)
{
	...
	case SND_SOC_DAIFMT_I2S:
		...
		break;
	default:
		aw_err("unsupported DAI format %d", fmt & SND_SOC_DAIFMT_FORMAT_MASK);
		return -EINVAL;
	}
...
static int aw869xx_set_dai_sysclk(struct snd_soc_dai *dai, int clk_id, uint32_t freq, int dir)
{
	aw_info("freq=%d", freq);
...
static int aw869xx_hw_params(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params,
			     struct snd_soc_dai *dai)
{
	...
	/* check i2s cfg */
	haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_I2SCFG1, &tmp_val, AW_I2C_BYTE_ONE);
...
static int aw869xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_SYSST, &reg_val, AW_I2C_BYTE_ONE);
		aw_info("sysst=0x%02X", reg_val);
		haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_SYSST2, &reg_val, AW_I2C_BYTE_ONE);
		aw_info("sysst2=0x%02X", reg_val);
		haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_SYSER, &reg_val, AW_I2C_BYTE_ONE);
...
static void aw869xx_shutdown(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	aw_snd_soc_codec_t *codec = aw_get_codec(dai);
	struct aw_haptic *aw_haptic = aw_componet_codec_ops.aw_snd_soc_codec_get_drvdata(codec);
...
static int aw869xx_codec_write(aw_snd_soc_codec_t *codec, uint32_t reg, uint32_t value)
{
...
#ifdef KERNEL_OVER_4_19
static const struct snd_soc_component_driver soc_codec_dev_aw869xx = {
...
static int aw869xx_snd_soc_init(struct device *dev)
{
	...
	ret = aw_componet_codec_ops.aw_snd_soc_register_codec(dev, &soc_codec_dev_aw869xx, dai,
							      ARRAY_SIZE(aw869xx_dai));
	if (ret < 0) {
...
static int aw869xx_check_qualify(struct aw_haptic *aw_haptic)
{
	...
	aw_info("enter");
	/* chip qualify */
	ret = haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_RDATA_A, &reg_val, AW_I2C_BYTE_ONE);
	if (ret < 0)
		return ret;
	if (!(reg_val & 0x80)) {
...
{
	haptic_hv_i2c_writes(aw_haptic, AW869XX_REG_PLAYCFG2, &gain, AW_I2C_BYTE_ONE);
}
...
static void aw869xx_set_bst_vol(struct aw_haptic *aw_haptic, uint32_t bst_vol)
{
	...
	reg_val = AW869XX_BST_VOL_FORMULA(bst_vol);
	haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_PLAYCFG1,
				 AW869XX_BIT_PLAYCFG1_BST_VOUT_RDA_MASK, reg_val);
}

static void aw869xx_set_wav_seq(struct aw_haptic *aw_haptic, uint8_t wav, uint8_t seq)
{
	haptic_hv_i2c_writes(aw_haptic, AW869XX_REG_WAVCFG1 + wav, &seq, AW_I2C_BYTE_ONE);
}

static void aw869xx_set_wav_loop(struct aw_haptic *aw_haptic, uint8_t wav, uint8_t loop)
{
	uint8_t tmp = 0;
...
static void aw869xx_set_rtp_data(struct aw_haptic *aw_haptic, uint8_t *data, uint32_t len)
{
...
static void aw869xx_set_ram_addr(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW869XX_REG_RAMADDRH, ram_addr, AW_I2C_BYTE_TWO);
}
...
static uint8_t aw869xx_get_glb_state(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_GLBRD5, &state, AW_I2C_BYTE_ONE);
...
static void aw869xx_play_go(struct aw_haptic *aw_haptic, bool flag)
{
	...
	if (flag) {
		...
		haptic_hv_i2c_writes(aw_haptic, AW869XX_REG_PLAYCFG4, &stop_on, AW_I2C_BYTE_ONE);
	}
}

static void aw869xx_bst_mode_config(struct aw_haptic *aw_haptic, uint8_t boost_mode)
{
	switch (boost_mode) {
...
static void aw869xx_stop(struct aw_haptic *aw_haptic)
{
	...
	reg_val = AW869XX_BIT_PLAYCFG4_GO_ON;
	haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_PLAYCFG3,
					AW869XX_BIT_PLAYCFG3_PLAY_MODE_MASK,
					AW869XX_BIT_PLAYCFG3_PLAY_MODE_STOP);
	haptic_hv_i2c_writes(aw_haptic, AW869XX_REG_PLAYCFG4, &reg_val, AW_I2C_BYTE_ONE);
...
static uint8_t aw869xx_get_trim_lra(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_TRIMCFG3, &reg_val, AW_I2C_BYTE_ONE);
	reg_val &= 0x3F;
...
static void aw869xx_vbat_mode_config(struct aw_haptic *aw_haptic, uint8_t flag)
{
	...
	if (flag == AW_CONT_VBAT_HW_COMP_MODE) {
		...
		haptic_hv_i2c_writes(aw_haptic, AW869XX_REG_GLBCFG2, &reg_val, AW_I2C_BYTE_ONE);
		haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_SYSCTRL1,
...
static void aw869xx_protect_config(struct aw_haptic *aw_haptic, uint8_t prtime, uint8_t prlvl)
{
	uint8_t reg_val = 0;

	haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_PWMCFG1, AW869XX_BIT_PWMCFG1_PRC_EN_MASK,
	...
	if (prlvl != 0) {
		...
		haptic_hv_i2c_writes(aw_haptic, AW869XX_REG_PWMCFG3, &reg_val, AW_I2C_BYTE_ONE);
		haptic_hv_i2c_writes(aw_haptic, AW869XX_REG_PWMCFG4, &prtime, AW_I2C_BYTE_ONE);
	} else {
		/* Disable */
...
static void aw869xx_cont_config(struct aw_haptic *aw_haptic)
{
	...
	/* DRV2_TIME */
	haptic_hv_i2c_writes(aw_haptic, AW869XX_REG_CONTCFG9, &drv2_time, AW_I2C_BYTE_ONE);
	/* cont play go */
...
static void aw869xx_one_wire_init(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW869XX_REG_GLBCFG4, &trig_prio, AW_I2C_BYTE_ONE);
	haptic_hv_i2c_writes(aw_haptic, AW869XX_REG_GLBCFG2, &delay_2p5ms, AW_I2C_BYTE_ONE);
...
{
	aw_info("enter");
	haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_SYSCTRL2, AW869XX_BIT_SYSCTRL2_I2S_PIN_MASK,
				 AW869XX_BIT_SYSCTRL2_I2S_PIN_I2S);
	haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_IOCFG1, AW869XX_BIT_IOCFG1_IO_FAST_MASK,
...
static void aw869xx_trig1_param_config(struct aw_haptic *aw_haptic)
{
	...
	if (aw_haptic->trig[0].pos_enable) {
		...
		haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_TRGCFG1,
					 AW869XX_BIT_TRG_ENABLE_MASK, AW869XX_BIT_TRG_DISABLE);
	}
	...
	if (aw_haptic->trig[0].pos_sequence) {
		haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_TRGCFG1, AW869XX_BIT_TRG_SEQ_MASK,
					 aw_haptic->trig[0].pos_sequence);
	}
	if (aw_haptic->trig[0].neg_sequence) {
		haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_TRGCFG4, AW869XX_BIT_TRG_SEQ_MASK,
...
static void aw869xx_trig2_param_config(struct aw_haptic *aw_haptic)
{
	...
	if (aw_haptic->trig[1].pos_sequence) {
		haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_TRGCFG2, AW869XX_BIT_TRG_SEQ_MASK,
					 aw_haptic->trig[1].pos_sequence);
	}
	if (aw_haptic->trig[1].neg_sequence) {
		haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_TRGCFG5, AW869XX_BIT_TRG_SEQ_MASK,
...
static void aw869xx_trig3_param_config(struct aw_haptic *aw_haptic)
{
	...
	if (aw_haptic->trig[2].pos_sequence) {
		haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_TRGCFG3, AW869XX_BIT_TRG_SEQ_MASK,
					 aw_haptic->trig[2].pos_sequence);
	}
	if (aw_haptic->trig[2].neg_sequence) {
		haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_TRGCFG6, AW869XX_BIT_TRG_SEQ_MASK,
...
static void aw869xx_get_ram_data(struct aw_haptic *aw_haptic, char *buf)
{
	...
	while (i < aw_haptic->ram.len) {
		...
		haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_RAMDATA, buf + i, size);
		i += size;
	}
}

static void aw869xx_get_first_wave_addr(struct aw_haptic *aw_haptic, uint8_t *wave_addr)
{
	uint8_t reg_array[3] = {0};

	haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_RAMDATA, reg_array, AW_I2C_BYTE_THREE);
...
static size_t aw869xx_get_wav_loop(struct aw_haptic *aw_haptic, char *buf)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_WAVCFG9, reg_val, AW_I2C_BYTE_FOUR);
...
static uint8_t aw869xx_get_prctmode(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_PWMCFG3, &reg_val, AW_I2C_BYTE_ONE);
	reg_val >>= 7;
...
static int aw869xx_read_f0(struct aw_haptic *aw_haptic)
{
	...
	/* lra_f0 */
	haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_CONTRD14, reg_val, AW_I2C_BYTE_TWO);
	...
	aw_haptic->f0 = (uint32_t)AW869XX_F0_FORMULA(f0_reg);
	aw_info("lra_f0=%d", aw_haptic->f0);
#else
	/* cont_f0 */
	haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_CONTRD16, reg_val, AW_I2C_BYTE_TWO);
	f0_reg = (reg_val[0] << 8) | (reg_val[1] << 0);
	if (!f0_reg) {
		aw_haptic->f0 = 0;
...
static int aw869xx_get_f0(struct aw_haptic *aw_haptic)
{
	...
	/* enter standby mode */
	aw869xx_stop(aw_haptic);
	haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_SYSCTRL7, AW869XX_BIT_SYSCTRL7_D2S_GAIN_MASK,
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_CONTCFG6,
				 ...
				 (aw_haptic->info.is_enabled_track_en << 7));
	/* enable auto break */
	haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_PLAYCFG3, &reg_val, AW_I2C_BYTE_ONE);
	brk_en_default = 0x04 & reg_val;
	aw869xx_auto_brake_mode(aw_haptic, true);
	/* f0 driver level & time */
	haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_CONTCFG6,
				 AW869XX_BIT_CONTCFG6_DRV1_LVL_MASK, aw_haptic->info.cont_drv1_lvl);
	...
	haptic_hv_i2c_writes(aw_haptic, AW869XX_REG_CONTCFG7, cont_config, AW_I2C_BYTE_THREE);
	/* TRACK_MARGIN */
	...
	haptic_hv_i2c_writes(aw_haptic, AW869XX_REG_CONTCFG3, &cont_config[0], AW_I2C_BYTE_ONE);
	/* play go */
	aw869xx_play_go(aw_haptic, true);
...
static int aw869xx_select_d2s_gain(uint8_t reg)
{
	int d2s_gain = 0;

	switch (reg) {
	case AW869XX_BIT_SYSCTRL7_D2S_GAIN_1:
		d2s_gain = 1;
		break;
	case AW869XX_BIT_SYSCTRL7_D2S_GAIN_2:
		d2s_gain = 2;
		break;
	case AW869XX_BIT_SYSCTRL7_D2S_GAIN_4:
		d2s_gain = 4;
		break;
	case AW869XX_BIT_SYSCTRL7_D2S_GAIN_8:
		d2s_gain = 8;
		break;
	case AW869XX_BIT_SYSCTRL7_D2S_GAIN_10:
		d2s_gain = 10;
		break;
	case AW869XX_BIT_SYSCTRL7_D2S_GAIN_16:
		d2s_gain = 16;
		break;
	case AW869XX_BIT_SYSCTRL7_D2S_GAIN_20:
		d2s_gain = 20;
		break;
	case AW869XX_BIT_SYSCTRL7_D2S_GAIN_26:
		d2s_gain = 26;
		break;
	default:
		d2s_gain = -1;
...
static void aw869xx_get_lra_resistance(struct aw_haptic *aw_haptic)
{
	...
	aw869xx_stop(aw_haptic);
	usleep_range(2000, 2500);
	haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_SYSCTRL2, AW869XX_BIT_SYSCTRL2_STANDBY_MASK,
				 AW869XX_BIT_SYSCTRL2_STANDBY_OFF);
	haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_SYSCTRL7, &reg_val, AW_I2C_BYTE_ONE);
	d2s_gain_default = reg_val & (~AW869XX_BIT_SYSCTRL7_D2S_GAIN_MASK);
	haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_SYSCTRL7,
					AW869XX_BIT_SYSCTRL7_D2S_GAIN_MASK,
					AW869XX_BIT_SYSCTRL7_D2S_GAIN_10);
	d2s_gain = aw869xx_select_d2s_gain(AW869XX_BIT_SYSCTRL7_D2S_GAIN_10);
	if (d2s_gain <= 0) {
		aw_err("d2s_gain is error");
		return;
	}
	haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_DETCFG1, AW869XX_BIT_DETCFG1_RL_OS_MASK,
				 AW869XX_BIT_DETCFG1_RL);
	haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_DETCFG2, AW869XX_BIT_DETCFG2_DIAG_GO_MASK,
				 AW869XX_BIT_DETCFG2_DIAG_GO_ON);
	usleep_range(30000, 35000);
	haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_DET_RL, &reg_val, AW_I2C_BYTE_ONE);
	lra_code = (lra_code | reg_val) << 2;
	haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_DET_LO, &reg_val, AW_I2C_BYTE_ONE);
	lra_code = lra_code | (reg_val & 0x03);
	aw_haptic->lra = AW869XX_LRA_FORMULA(lra_code, d2s_gain);
	aw869xx_ram_init(aw_haptic, false);
	haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_SYSCTRL7,
				 AW869XX_BIT_SYSCTRL7_D2S_GAIN_MASK, d2s_gain_default);
}
...
static void aw869xx_get_vbat(struct aw_haptic *aw_haptic)
{
	...
	aw869xx_stop(aw_haptic);
	aw869xx_ram_init(aw_haptic, true);
	haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_DETCFG2, AW869XX_BIT_DETCFG2_VBAT_GO_MASK,
				 AW869XX_BIT_DETCFG2_VABT_GO_ON);

	while (cont--) {
		haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_DETCFG2, &reg_val, AW_I2C_BYTE_ONE);
		if ((reg_val & 0x02) == 0 || cont == 0)
			break;
	}

	haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_DET_VBAT, &reg_val, AW_I2C_BYTE_ONE);
...
static ssize_t aw869xx_get_reg(struct aw_haptic *aw_haptic, ssize_t len, char *buf)
{
	uint8_t i = 0;
	uint8_t reg_array[AW869XX_REG_D2SCFG1 + 1] = {0};

	haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_ID, reg_array, AW869XX_REG_RTPDATA);
...
static int aw869xx_offset_cali(struct aw_haptic *aw_haptic)
{
	uint8_t reg_val[3] = {0};
	int os_code = 0;
	int d2s_gain = 0;
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_SYSCTRL7,
				 AW869XX_BIT_SYSCTRL7_D2S_GAIN_MASK, aw_haptic->info.d2s_gain);
	aw869xx_ram_init(aw_haptic, true);
	haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_DETCFG2, AW869XX_BIT_DETCFG2_DIAG_GO_MASK,
				 AW869XX_BIT_DETCFG2_DIAG_GO_ON);
	usleep_range(3000, 3500);
	haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_DET_OS, &reg_val[0], AW_I2C_BYTE_ONE);
	haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_DET_LO, &reg_val[1], AW_I2C_BYTE_ONE);
	aw869xx_ram_init(aw_haptic, false);
	d2s_gain = aw869xx_select_d2s_gain(aw_haptic->info.d2s_gain);
	if (d2s_gain <= 0) {
		aw_err("d2s_gain is error");
		return -ERANGE;
	}
	os_code = (reg_val[0] << 2) | ((reg_val[1] & (~AW869XX_BIT_DET_LO_OS_MASK)) >> 2);
	os_code = AW869XX_OS_FORMULA(os_code, d2s_gain);
	aw_info("os_code = %d", os_code);
...
static int aw869xx_check_ram_data(struct aw_haptic *aw_haptic, uint8_t *cont_data,
				    ...
				    struct aw_haptic_container *awinic_cont)
{
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_RTPCFG3,
				 (AW869XX_BIT_RTPCFG3_FIFO_AEH_MASK &
				  AW869XX_BIT_RTPCFG3_FIFO_AFH_MASK), (fifo_adr[2] | fifo_adr[3]));
	haptic_hv_i2c_writes(aw_haptic, AW869XX_REG_RTPCFG4, fifo_adr, AW_I2C_BYTE_TWO);
	haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_RTPCFG3, reg_val, AW_I2C_BYTE_THREE);
	...
	while (i < awinic_cont->len) {
		...
		haptic_hv_i2c_writes(aw_haptic, AW869XX_REG_RAMDATA, &awinic_cont->data[i], len);
		i += len;
	}
#ifdef AW_CHECK_RAM_DATA
#ifdef AW869XX_CHECK_RAM_DATA
	...
	while (i < awinic_cont->len) {
		...
		haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_RAMDATA, ram_data, len);
		ret = aw869xx_check_ram_data(aw_haptic, &awinic_cont->data[i], ram_data, len);
		if (ret < 0)
			break;
		i += len;
	}
...
static uint64_t aw869xx_get_theory_time(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_SYSCTRL2, &reg_val, AW_I2C_BYTE_ONE);
	fre_val = reg_val & (~AW869XX_BIT_SYSCTRL2_WAVDAT_MODE_MASK);

	if (fre_val == AW869XX_BIT_SYSCTRL2_RATE_12K)
		theory_time = aw_haptic->rtp_len * 1000 / 12;	/*12K */
	if (fre_val == AW869XX_BIT_SYSCTRL2_RATE_24K)
		theory_time = aw_haptic->rtp_len * 1000 / 24;	/*24K */
	if (fre_val == AW869XX_BIT_SYSCTRL2_RATE_48K)
			     ...
			     struct device_node *np)
{
	...
	val = of_property_read_u8(np, "aw869xx_cont_drv1_lvl", &aw_haptic->info.cont_drv1_lvl);
	if (val != 0)
		aw_info("aw869xx_cont_drv1_lvl not found");
	val = of_property_read_u32(np, "aw869xx_cont_lra_vrms", &aw_haptic->info.cont_lra_vrms);
	if (val != 0)
		aw_info("aw869xx_cont_lra_vrms not found");
	val = of_property_read_u8(np, "aw869xx_cont_drv1_time", &aw_haptic->info.cont_drv1_time);
	if (val != 0)
		aw_info("aw869xx_cont_drv1_time not found");
	val = of_property_read_u8(np, "aw869xx_cont_drv2_time", &aw_haptic->info.cont_drv2_time);
	if (val != 0)
		aw_info("aw869xx_cont_drv2_time not found");
	val = of_property_read_u8(np, "aw869xx_cont_brk_gain", &aw_haptic->info.cont_brk_gain);
	if (val != 0)
		aw_info("aw869xx_cont_brk_gain not found");
	val = of_property_read_u8(np, "aw869xx_d2s_gain", &aw_haptic->info.d2s_gain);
	if (val != 0)
		aw_info("aw869xx_d2s_gain not found");
	val = of_property_read_u8(np, "aw869xx_cont_brk_time", &aw_haptic->info.cont_brk_time);
	if (val != 0)
	...
	aw_haptic->info.is_enabled_i2s = of_property_read_bool(np, "aw869xx_is_enabled_i2s");
	aw_info("aw_haptic->info.is_enabled_i2s = %d", aw_haptic->info.is_enabled_i2s);
	...
	val = of_property_read_u32(np, "aw869xx_bst_vol_default", &aw_haptic->info.bst_vol_default);
	if (val != 0)
	...
	val = of_property_read_u8_array(np, "aw869xx_bstcfg", bstcfg_temp, ARRAY_SIZE(bstcfg_temp));
	if (val != 0)
		aw_info("aw869xx_bstcfg not found");
	else
		memcpy(aw_haptic->info.bstcfg, bstcfg_temp, sizeof(bstcfg_temp));
...
static void aw869xx_misc_para_init(struct aw_haptic *aw_haptic)
{
	...
	/* Cont drv2 lvl */
	drv2_lvl = AW869XX_DRV2_LVL_FORMULA(aw_haptic->info.f0_pre, aw_haptic->info.cont_lra_vrms);
	...
	/* Get gain */
	haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_PLAYCFG2, reg_val, AW_I2C_BYTE_ONE);
	aw_haptic->gain = reg_val[0];
	/* Get wave_seq */
	haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_WAVCFG1, reg_val, AW_I2C_BYTE_EIGHT);
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_CONTCFG5,
				 AW869XX_BIT_CONTCFG5_BRK_GAIN_MASK, aw_haptic->info.cont_brk_gain);
	/* i2s enbale */
	if (aw_haptic->info.is_enabled_i2s && aw_haptic->i2s_config) {
		aw_info("i2s enabled!");
		haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_SYSCTRL2,
	...
	/* set BST_ADJ */
	haptic_hv_i2c_write_bits(aw_haptic, AW869XX_REG_BSTCFG5, AW869XX_BIT_BSTCFG5_BST_ADJ_MASK,
				   ...
				   const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		haptic_hv_i2c_writes(aw_haptic, AW869XX_REG_CONTCFG8, cont_time, AW_I2C_BYTE_TWO);
	}
...
static int aw869xx_creat_node(struct aw_haptic *aw_haptic)
{
	...
	ret = sysfs_create_group(&aw_haptic->vib_dev.dev->kobj, &aw869xx_vibrator_attribute_group);
	if (ret < 0)
