...
#ifdef AW_DOUBLE
extern struct aw_haptic *left;
extern struct aw_haptic *right;
#endif
/******************************************************
 *
 * aw8693x codec
...
static void aw8693x_set_pwm(struct aw_haptic *aw_haptic, uint8_t mode)
{
	...
	case AW_PWM_8K:
		haptic_hv_i2c_write_bits(aw_haptic, AW8693X_REG_SYSCTRL4,
					 AW8693X_BIT_SYSCTRL4_WAVDAT_MODE_MASK,
					 AW8693X_BIT_SYSCTRL4_WAVDAT_8K);
		break;
	default:
		aw_err("error param");
		break;
...
static void aw8693x_vbat_ref_config(struct aw_haptic *aw_haptic, uint16_t vbat_ref)
{
	uint8_t reg_val = 0;

	aw_info("enter");
	if (vbat_ref <= 3450)
		reg_val = AW8693X_BIT_DETCFG1_VBAT_REF_3_3;
	else if (vbat_ref <= 3800)
		reg_val = AW8693X_BIT_DETCFG1_VBAT_REF_3_6;
	else if (vbat_ref <= 4100)
		reg_val = AW8693X_BIT_DETCFG1_VBAT_REF_4_0;
	else if (vbat_ref <= 4350)
		reg_val = AW8693X_BIT_DETCFG1_VBAT_REF_4_2;
	else if (vbat_ref <= 4700)
		reg_val = AW8693X_BIT_DETCFG1_VBAT_REF_4_5;
	else if (vbat_ref <= 4900)
		reg_val = AW8693X_BIT_DETCFG1_VBAT_REF_4_8;
	else if (vbat_ref <= 5250)
		reg_val = AW8693X_BIT_DETCFG1_VBAT_REF_5_0;
	else
		reg_val = AW8693X_BIT_DETCFG1_VBAT_REF_5_5;

	haptic_hv_i2c_write_bits(aw_haptic, AW8693X_REG_DETCFG1,
					AW8693X_BIT_DETCFG1_VBAT_REF_MASK, reg_val);
}
...
static void aw8693x_stop(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8693X_REG_PLAYCFG3,
					AW8693X_BIT_PLAYCFG3_PLAY_MODE_MASK,
					AW8693X_BIT_PLAYCFG3_PLAY_MODE_STOP);
	haptic_hv_i2c_writes(aw_haptic, AW8693X_REG_PLAYCFG4, &reg_val, AW_I2C_BYTE_ONE);
...
static int aw8693x_get_f0(struct aw_haptic *aw_haptic)
{
	...
	aw8693x_vbat_ref_config(aw_haptic, AW_CONT_F0_VBAT_REFER);
	...
	aw8693x_vbat_ref_config(aw_haptic, AW_VBAT_REFER);
...
static void aw8693x_get_lra_resistance(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8693X_REG_DETCFG2,
					AW8693X_BIT_DETCFG2_D2S_GAIN_MASK,
					AW8693X_BIT_DETCFG2_D2S_GAIN_10);
	d2s_gain = aw8693x_select_d2s_gain(AW8693X_BIT_DETCFG2_D2S_GAIN_10);
	if (d2s_gain <= 0) {
...
static int aw8693x_offset_cali(struct aw_haptic *aw_haptic)
{
	...
	aw8693x_ram_init(aw_haptic, false);
	d2s_gain = aw8693x_select_d2s_gain(aw_haptic->info.d2s_gain);
	if (d2s_gain <= 0) {
		aw_err("d2s_gain is error");
		return -ERANGE;
	}
			     ...
			     struct device_node *np)
{
	...
	val = of_property_read_u8(np, "aw8693x_uvlo_vol_default", &aw_haptic->info.uvlo_vol_default);
	if (val != 0)
		aw_info("aw8693x_uvlo_vol_default not found");
...
static void aw8693x_misc_para_init(struct aw_haptic *aw_haptic)
{
	...
	/* record I2C default addr */
	aw_haptic->record_i2c_addr = aw_haptic->i2c->addr;
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8693X_REG_SYSCTRL6,
				 AW8693X_BIT_SYSCTRL6_UVLO_ADJ_MASK,
				 aw_haptic->info.uvlo_vol_default);
 ...
 ******************************************************/
#ifdef DUAL_RTP_TEST
static ssize_t rtp_dual_test_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t rtp_dual_test_store(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t count)
{
	...
	right->dual_flag = false;
	if ((val > 0) && (val < right->rtp_num)) {
		aw8693x_set_pwm(left, AW_PWM_12K);
		aw8693x_set_pwm(right, AW_PWM_12K);
		right->state = 1;
		right->rtp_file_num = val;
		...
		haptic_hv_i2c_write_bits(right, AW8693X_REG_SYSCTRL5,
							...
							AW8693X_BROADCAST_ADDR);
		reg = 0x68;
		haptic_hv_i2c_writes(right, AW8693X_REG_SYSCTRL2, &reg, AW_I2C_BYTE_ONE);

		/* change broadcast addr */
		right->i2c->addr = AW8693X_BROADCAST_ADDR;
		haptic_hv_i2c_write_bits(right, AW8693X_REG_RTPCFG1,
		...
		/* close dual rtp */
		right->i2c->addr = AW8693X_BROADCAST_ADDR;
		aw8693x_set_pwm(right, AW_PWM_24K);
		haptic_hv_i2c_write_bits(right, AW8693X_REG_RTPCFG1,
							AW8693X_BIT_RTPCFG1_TWORTP_EN_MASK,
		...
		/* restore i2c addr */
		right->i2c->addr = right->record_i2c_addr;
		aw_info("restore i2c addr 0x%0x", right->i2c->addr);
	} else {
		right->state = 0;
...
#ifdef DUAL_RTP_TEST
static DEVICE_ATTR_RW(rtp_dual_test);
#endif
