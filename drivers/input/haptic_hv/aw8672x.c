...
static void aw8672x_stop(struct aw_haptic *aw_haptic)
{
	...
	aw_info("enter");
	aw_haptic->play_mode = AW_STANDBY_MODE;
	reg_val = AW8672X_BIT_PLAYCFG4_GO_ON;
	haptic_hv_i2c_write_bits(aw_haptic, AW8672X_REG_PLAYCFG3,
					AW8672X_BIT_PLAYCFG3_PLAY_MODE_MASK,
					AW8672X_BIT_PLAYCFG3_PLAY_MODE_STOP);
	haptic_hv_i2c_writes(aw_haptic, AW8672X_REG_PLAYCFG4, &reg_val, AW_I2C_BYTE_ONE);
	ret = aw8672x_wait_enter_standby(aw_haptic);
	if (ret < 0) {
...
static void aw8672x_vbat_ref_config(struct aw_haptic *aw_haptic, uint16_t vbat_ref)
{
	uint8_t reg_val = 0;

	aw_info("enter");
	if (vbat_ref <= 3450)
		reg_val = AW8672X_BIT_DETCFG1_VBAT_REF_3_3;
	else if (vbat_ref <= 3800)
		reg_val = AW8672X_BIT_DETCFG1_VBAT_REF_3_6;
	else if (vbat_ref <= 4100)
		reg_val = AW8672X_BIT_DETCFG1_VBAT_REF_4_0;
	else if (vbat_ref <= 4350)
		reg_val = AW8672X_BIT_DETCFG1_VBAT_REF_4_2;
	else if (vbat_ref <= 4700)
		reg_val = AW8672X_BIT_DETCFG1_VBAT_REF_4_5;
	else if (vbat_ref <= 4900)
		reg_val = AW8672X_BIT_DETCFG1_VBAT_REF_4_8;
	else if (vbat_ref <= 5250)
		reg_val = AW8672X_BIT_DETCFG1_VBAT_REF_5_0;
	else
		reg_val = AW8672X_BIT_DETCFG1_VBAT_REF_5_5;

	haptic_hv_i2c_write_bits(aw_haptic, AW8672X_REG_DETCFG1,
					AW8672X_BIT_DETCFG1_VBAT_REF_MASK, reg_val);
}
...
static int aw8672x_get_f0(struct aw_haptic *aw_haptic)
{
	...
	aw8672x_vbat_ref_config(aw_haptic, AW_CONT_F0_VBAT_REFER);
	/* config dts d2s_gain */
	...
	aw8672x_vbat_ref_config(aw_haptic, AW_VBAT_REFER);
...
static void aw8672x_get_lra_resistance(struct aw_haptic *aw_haptic)
{
	...
	d2s_gain_default = reg_val[0] & (~AW8672X_BIT_DETCFG2_D2S_GAIN_MASK);
	haptic_hv_i2c_write_bits(aw_haptic, AW8672X_REG_DETCFG2,
					AW8672X_BIT_DETCFG2_D2S_GAIN_MASK,
					AW8672X_BIT_DETCFG2_D2S_GAIN_10);
	d2s_gain = aw8672x_select_d2s_gain(AW8672X_BIT_DETCFG2_D2S_GAIN_10);
	if (d2s_gain <= 0) {
...
static int aw8672x_offset_cali(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8672X_REG_DETRD1, reg_val, AW_I2C_BYTE_TWO);
	aw8672x_ram_init(aw_haptic, false);
	d2s_gain = aw8672x_select_d2s_gain(aw_haptic->info.d2s_gain);
	if (d2s_gain <= 0) {
		aw_err("d2s_gain is error");
		return -ERANGE;
	}
			    ...
			    struct device_node *np)
{
	...
	val = of_property_read_u8(np, "aw8672x_uvlo_vol_default", &aw_haptic->info.uvlo_vol_default);
	if (val != 0)
		aw_info("aw8672x_uvlo_vol_default not found");
...
static void aw8672x_misc_para_init(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8672X_REG_UVLOCFG,
				 AW8672X_BIT_UVLOCFG_UVLO_ADJ_MASK,
				 aw_haptic->info.uvlo_vol_default);
	/* PD BEMF DOUBLE */
	haptic_hv_i2c_write_bits(aw_haptic, AW8672X_REG_ANACFG3,
				AW8672X_BIT_ANACFG3_PD_BEMF_MASK,
				AW8672X_BIT_ANACFG3_PD_BEMF_HDP_HDN);
