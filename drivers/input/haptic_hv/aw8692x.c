...
static void aw8692x_reg_unlock(struct aw_haptic *aw_haptic, bool flag)
{
	...
	if (flag) {
		...
		haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_TMCFG, &reg_val, AW_I2C_BYTE_ONE);
	} else {
		/* Lock register */
...
static int aw8692x_check_qualify(struct aw_haptic *aw_haptic)
{
	...
	aw_info("enter");
	haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_EFCFG6, reg_val, AW_I2C_BYTE_THREE);
	...
	if ((reg_val[1] & (~AW8692X_BIT_EFCFG7_RESERVED_MASK)) ||
		...
		haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_EFCFG1, reg_val, AW_I2C_BYTE_ONE);
		usleep_range(2500, 3000);
		aw8692x_ram_init(aw_haptic, false);
	}
	haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_EFCFG7, &reg_val[1], AW_I2C_BYTE_TWO);
	if ((reg_val[1] & (~AW8692X_BIT_EFCFG7_RESERVED_MASK)) ||
	    (reg_val[2] & (~AW8692X_BIT_EFCFG8_EF_TRM_BST_IPEAK_MASK))) {
		return -ERANGE;
	}
	reg_val[0] = AW8692X_BIT_ANACFG2_INIT_VAL;
	haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_ANACFG2, reg_val, AW_I2C_BYTE_ONE);
	haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_EFCFG9, &reg_val[0], AW_I2C_BYTE_ONE);
...
static void aw8692x_set_wav_seq(struct aw_haptic *aw_haptic, uint8_t wav, uint8_t seq)
{
	haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_WAVCFG1 + wav, &seq, AW_I2C_BYTE_ONE);
}

static void aw8692x_set_wav_loop(struct aw_haptic *aw_haptic, uint8_t wav, uint8_t loop)
{
	uint8_t tmp = 0;
...
static void aw8692x_set_rtp_data(struct aw_haptic *aw_haptic, uint8_t *data, uint32_t len)
{
...
static void aw8692x_set_ram_addr(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_RAMADDRH, ram_addr, AW_I2C_BYTE_TWO);
}
...
static void aw8692x_set_base_addr(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_RTPCFG2, &rtp_addr[1], AW_I2C_BYTE_ONE);
}

static void aw8692x_auto_brake_mode(struct aw_haptic *aw_haptic, bool flag)
{
	if (flag) {
...
static uint8_t aw8692x_get_glb_state(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_GLBRD5, &state, AW_I2C_BYTE_ONE);
...
static void aw8692x_play_go(struct aw_haptic *aw_haptic, bool flag)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_ANACFG20, &reg_val, AW_I2C_BYTE_ONE);
	if (flag && (reg_val <= AW8692X_BIT_ANACFG20_TRIM_LRA))
		haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_PLAYCFG4, &go_on, AW_I2C_BYTE_ONE);
	else
		haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_PLAYCFG4, &stop_on, AW_I2C_BYTE_ONE);
}
...
static void aw8692x_play_mode(struct aw_haptic *aw_haptic, uint8_t play_mode)
{
	...
	case AW_RAM_MODE:
		...
		aw8692x_auto_brake_mode(aw_haptic, false);
	...
	case AW_RAM_LOOP_MODE:
		...
		haptic_hv_i2c_write_bits(aw_haptic, AW8692X_REG_PLAYCFG3,
					 AW8692X_BIT_PLAYCFG3_PLAY_MODE_MASK,
					 AW8692X_BIT_PLAYCFG3_PLAY_MODE_RAM);
		aw8692x_auto_brake_mode(aw_haptic, true);
...
static void aw8692x_stop(struct aw_haptic *aw_haptic)
{
	...
	aw_info("enter");
	aw_haptic->play_mode = AW_STANDBY_MODE;
	reg_val = AW8692X_BIT_PLAYCFG4_GO_ON;
	haptic_hv_i2c_write_bits(aw_haptic, AW8692X_REG_PLAYCFG3,
					AW8692X_BIT_PLAYCFG3_PLAY_MODE_MASK,
					AW8692X_BIT_PLAYCFG3_PLAY_MODE_STOP);
	haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_PLAYCFG4, &reg_val, AW_I2C_BYTE_ONE);
...
static void aw8692x_upload_lra(struct aw_haptic *aw_haptic, uint32_t flag)
{
	...
	case AW_F0_CALI_LRA:
		...
		cali_data = (char)aw_haptic->f0_cali_data & AW8692X_BIT_ANACFG20_TRIM_LRA;
		break;
	case AW_OSC_CALI_LRA:
		aw_info("write osc_cali_data to trim_lra = 0x%02X", aw_haptic->osc_cali_data);
		cali_data = (char)aw_haptic->osc_cali_data & AW8692X_BIT_ANACFG20_TRIM_LRA;
		break;
	default:
	...
	haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_ANACFG20, &cali_data, AW_I2C_BYTE_ONE);
	aw8692x_reg_unlock(aw_haptic, false);
}
...
static void aw8692x_protect_config(struct aw_haptic *aw_haptic, uint8_t prtime, uint8_t prlvl)
{
	uint8_t reg_val = 0;

	haptic_hv_i2c_write_bits(aw_haptic, AW8692X_REG_PWMCFG1, AW8692X_BIT_PWMCFG1_PRC_EN_MASK,
	...
	if (prlvl != 0) {
		...
		haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_PWMCFG3, &reg_val, AW_I2C_BYTE_ONE);
		haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_PWMCFG4, &prtime, AW_I2C_BYTE_ONE);
	} else {
		/* Disable */
...
static void aw8692x_cont_config(struct aw_haptic *aw_haptic)
{
	...
	/* DRV2_TIME */
	haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_CONTCFG9, &drv2_time, AW_I2C_BYTE_ONE);
	/* cont play go */
...
static void aw8692x_one_wire_init(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_GLBCFG4, &trig_prio, AW_I2C_BYTE_ONE);
	haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_GLBCFG2, &delay_2p5ms, AW_I2C_BYTE_ONE);
...
static void aw8692x_trig1_param_config(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_TRGCFG1, &trig_config, AW_I2C_BYTE_ONE);
	...
	haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_TRGCFG4, &trig_config, AW_I2C_BYTE_ONE);
}
...
static void aw8692x_trig2_param_config(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_TRGCFG2, &trig_config, AW_I2C_BYTE_ONE);
	...
	haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_TRGCFG5, &trig_config, AW_I2C_BYTE_ONE);
}
...
static void aw8692x_trig3_param_config(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_TRGCFG3, &trig_config, AW_I2C_BYTE_ONE);
	...
	haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_TRGCFG6, &trig_config, AW_I2C_BYTE_ONE);
}
...
static void aw8692x_get_ram_data(struct aw_haptic *aw_haptic, char *buf)
{
	...
	while (i < aw_haptic->ram.len) {
		...
		haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_RAMDATA, buf + i, size);
		i += size;
	}
}

static void aw8692x_get_first_wave_addr(struct aw_haptic *aw_haptic, uint8_t *wave_addr)
{
	uint8_t reg_val[3] = {0};

	haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_RAMDATA, reg_val, AW_I2C_BYTE_THREE);
...
static size_t aw8692x_get_wav_loop(struct aw_haptic *aw_haptic, char *buf)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_WAVCFG9, reg_val, AW_SEQUENCER_LOOP_SIZE);
...
static uint8_t aw8692x_get_prctmode(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_PWMCFG3, &reg_val, AW_I2C_BYTE_ONE);
	reg_val >>= 7;
...
static int aw8692x_read_f0(struct aw_haptic *aw_haptic)
{
	...
	/* lra_f0 */
	haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_CONTCFG14, reg_val, AW_I2C_BYTE_TWO);
...
static int aw8692x_get_f0(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8692X_REG_DETCFG2, AW8692X_BIT_DETCFG2_D2S_GAIN_MASK,
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8692X_REG_CONTCFG6,
				 ...
				 (aw_haptic->info.is_enabled_track_en << 7));
	/* enable auto break */
	haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_PLAYCFG3, &reg_val, AW_I2C_BYTE_ONE);
	brk_en_default = 0x04 & reg_val;
	aw8692x_auto_brake_mode(aw_haptic, true);
	/* f0 driver level & time */
	haptic_hv_i2c_write_bits(aw_haptic, AW8692X_REG_CONTCFG6,
				 AW8692X_BIT_CONTCFG6_DRV1_LVL_MASK, aw_haptic->info.cont_drv1_lvl);
	...
	haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_CONTCFG7, cont_config, AW_I2C_BYTE_THREE);
	/* TRACK_MARGIN */
...
static uint8_t aw8692x_rtp_get_fifo_afs(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_SYSST, &reg_val, AW_I2C_BYTE_ONE);
...
static uint8_t aw8692x_get_osc_status(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_SYSST2, &state, AW_I2C_BYTE_ONE);
...
static void aw8692x_get_lra_resistance(struct aw_haptic *aw_haptic)
{
	...
	aw8692x_ram_init(aw_haptic, true);
	aw8692x_stop(aw_haptic);
	haptic_hv_i2c_write_bits(aw_haptic, AW8692X_REG_DETCFG2, AW8692X_BIT_DETCFG2_DET_SEQ0_MASK,
				 AW8692X_BIT_DETCFG2_DET_SEQ0_RL);
	haptic_hv_i2c_reads(aw_haptic,  AW8692X_REG_DETCFG1, &reg_val, AW_I2C_BYTE_ONE);
	adc_fs_default = reg_val & AW8692X_BIT_DETCFG1_ADC_FS;
	haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_DETCFG2, &reg_val, AW_I2C_BYTE_ONE);
	d2s_gain_default = reg_val & (~AW8692X_BIT_DETCFG2_D2S_GAIN_MASK);
	haptic_hv_i2c_write_bits(aw_haptic, AW8692X_REG_DETCFG2,
					AW8692X_BIT_DETCFG2_D2S_GAIN_MASK,
					AW8692X_BIT_DETCFG2_D2S_GAIN_10);
	d2s_gain = aw8692x_select_d2s_gain(AW8692X_BIT_DETCFG2_D2S_GAIN_10);
	if (d2s_gain <= 0) {
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8692X_REG_DETCFG1, AW8692X_BIT_DETCFG1_DET_GO_MASK,
				 AW8692X_BIT_DETCFG1_DET_GO_NA);
	/* restore default config*/
	aw8692x_ram_init(aw_haptic, false);
	haptic_hv_i2c_write_bits(aw_haptic, AW8692X_REG_DETCFG1, AW8692X_BIT_DETCFG1_ADC_FS_MASK,
				 adc_fs_default);
	haptic_hv_i2c_write_bits(aw_haptic, AW8692X_REG_DETCFG2,
				 AW8692X_BIT_DETCFG2_D2S_GAIN_MASK, d2s_gain_default);
	haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_DETRD1, reg_array, AW_I2C_BYTE_TWO);
	lra_code = ((reg_array[0] & AW8692X_BIT_DETRD1_AVG_DATA) << 8) + reg_array[1];
...
static void aw8692x_get_vbat(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8692X_REG_DETCFG1, AW8692X_BIT_DETCFG1_DET_GO_MASK,
				 AW8692X_BIT_DETCFG1_DET_GO_NA);
	haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_DETRD1, reg_array, AW_I2C_BYTE_TWO);
	vbat_code = ((reg_array[0] & AW8692X_BIT_DETRD1_AVG_DATA) << 8) + reg_array[1];
...
static ssize_t aw8692x_get_reg(struct aw_haptic *aw_haptic, ssize_t len, char *buf)
{
	uint8_t i = 0;
	uint8_t reg_array[AW8692X_REG_ANACFG22 + 1] = {0};

	haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_RSTCFG, reg_array, AW8692X_REG_RTPDATA);
...
static int aw8692x_offset_cali(struct aw_haptic *aw_haptic)
{
	return 0;
}
...
static int aw8692x_check_ram_data(struct aw_haptic *aw_haptic, uint8_t *cont_data,
				    ...
				    struct aw_haptic_container *awinic_cont)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_RTPCFG3, reg_array, AW_I2C_BYTE_THREE);

	/* get FIFO_AE and FIFO_AF addr */
	haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_RTPCFG3, reg_array, AW_I2C_BYTE_THREE);
	ae_addr_h = ((reg_array[0]) & AW8692X_BIT_RTPCFG3_FIFO_AEH) >> 4;
	ae_addr_l = reg_array[1];
	aw_info("almost_empty_threshold = %d", (uint16_t)((ae_addr_h << 8) | ae_addr_l));
	af_addr_h = ((reg_array[0]) & AW8692X_BIT_RTPCFG3_FIFO_AFH);
	af_addr_l = reg_array[2];
	aw_info("almost_full_threshold = %d", (uint16_t)((af_addr_h << 8) | af_addr_l));
	...
	while (i < awinic_cont->len) {
		...
		haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_RAMDATA, &awinic_cont->data[i], len);
		i += len;
	}
	...
	while (i < awinic_cont->len) {
		...
		haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_RAMDATA, ram_data, len);
		ret = aw8692x_check_ram_data(aw_haptic, &awinic_cont->data[i], ram_data, len);
		if (ret < 0)
			break;
		i += len;
	}
...
static uint64_t aw8692x_get_theory_time(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_SYSCTRL4, &reg_val, AW_I2C_BYTE_ONE);
	fre_val = reg_val & (~AW8692X_BIT_SYSCTRL4_WAVDAT_MODE_MASK);
	if (fre_val == AW8692X_BIT_SYSCTRL4_WAVDAT_12K)
		theory_time = aw_haptic->rtp_len * 1000 / 12;	/*12K*/
	if (fre_val == AW8692X_BIT_SYSCTRL4_WAVDAT_24K)
		theory_time = aw_haptic->rtp_len * 1000 / 24;	/*24K*/
	if (fre_val == AW8692X_BIT_SYSCTRL4_WAVDAT_48K)
			     ...
			     struct device_node *np)
{
	...
	val = of_property_read_u8(np, "aw8692x_gain_bypass", &aw_haptic->info.gain_bypass);
	if (val != 0)
	...
	if (val != 0)
		aw_info("aw8692x_trig_config not found");
	else
		memcpy(aw_haptic->info.trig_cfg, trig_config_temp, sizeof(trig_config_temp));
	val = of_property_read_u32(np, "aw8692x_bst_vol_default", &aw_haptic->info.bst_vol_default);
	if (val != 0)
		aw_info("aw8692x_bst_vol_default not found");
...
static void aw8692x_misc_para_init(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_SYSCTRL5, reg_val, AW_I2C_BYTE_ONE);
	/* Cont drv2 lvl */
	drv2_lvl = AW8692X_DRV2_LVL_FORMULA(aw_haptic->info.f0_pre, aw_haptic->info.cont_lra_vrms);
	...
	/* Get gain */
	haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_PLAYCFG2, reg_val, AW_I2C_BYTE_ONE);
	aw_haptic->gain = reg_val[0];
	/* Get wave_seq */
	haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_WAVCFG1, reg_val, AW_I2C_BYTE_EIGHT);
	...
	haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_ANACFG11, reg_val, AW_I2C_BYTE_ONE);
	/* Close boost skip */
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8692X_REG_ANACFG16, AW8692X_BIT_ANACFG16_BST_SRC_MASK,
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8692X_REG_CONTCFG5,
				 AW8692X_BIT_CONTCFG5_BRK_GAIN_MASK, aw_haptic->info.cont_brk_gain);
				  ...
				  const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		mutex_lock(&aw_haptic->lock);
		haptic_hv_i2c_reads(aw_haptic, AW8692X_REG_CONTCFG6, reg_array, AW_I2C_BYTE_ONE);
		...
		haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_CONTCFG6, reg_array, AW_I2C_BYTE_TWO);
		mutex_unlock(&aw_haptic->lock);
	}
				   ...
				   const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		mutex_lock(&aw_haptic->lock);
		haptic_hv_i2c_writes(aw_haptic, AW8692X_REG_CONTCFG8, reg_array, AW_I2C_BYTE_TWO);
		mutex_unlock(&aw_haptic->lock);
	}
...
static int aw8692x_creat_node(struct aw_haptic *aw_haptic)
{
	...
	ret = sysfs_create_group(&aw_haptic->vib_dev.dev->kobj, &aw8692x_vibrator_attribute_group);
	if (ret < 0) {
